...
- `tenancy` — Documents how Taskforge resolves multi-tenant scope via TenantContext and TenancyMiddleware (project routes, x-tenant-id, session), org membership checks, route model binding, and tests. Use when implementing or debugging tenant-scoped routes, APIs, policies, actions, Organization/Project/Task resolution, session tenant switching, or any work that requires tenancy context.
